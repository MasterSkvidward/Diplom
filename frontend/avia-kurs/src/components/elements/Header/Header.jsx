...
export default function Header() {
  ...
  useEffect(() => {
    // if (isAuth && !modalVisible) toast.success('Успешная авторизация!');
  }, [modalVisible])
  ...
  return (
    <header className={classes.header}>
      ...
      <div className={classes.header__container}>
        <h1 className={classes.brand_name} onClick={() => navigate(`/`)}>UmSchool</h1>
        <ul className={classes.header__links}>
          <li className={classes.header__link} onClick={() => navigate(`/course`)}>Курс</li>
          <li className={classes.header__link} onClick={() => navigate(`/messanger`)}>Мессенджер</li>
        </ul>
        <div className={classes.header__btn}>
          {
            isAuth
              ? <div className={classes.user}>
                ...
                <img src={user.img} alt="Profile" onClick={handleProfileClick} />
